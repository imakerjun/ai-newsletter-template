...
- **‘자세히 보기’(detail)는 풀어서 + 예시로**: 특히 **비개발자를 위한 AI 소식**의 `detail`은 한 줄로 끝내지 말고,
  ① 쉬운 말로 ‘이게 왜 중요한지’를 풀고 ② <b>예를 들어</b>로 시작하는 **구체적 예시 + 따라 할 샘플 한마디**
  (예: ‘이 메모를 결정·담당·기한 표로 정리해줘’)를 넣는다. HTML로 `<p>설명</p><p><b>예를 들어</b> …</p>` 2단락 권장.
  전문 용어는 풀어 쓰고, 코드 없이 바로 해볼 수 있는 행동으로 끝맺는다.
